...
kind: feature
status: active
summary: LiveView page for querying the knowledge base, with segment selection, ask/search mode toggle (defaulting to ask), and result display. Results are fetched asynchronously via start_async and rendered on completion.
surface:
