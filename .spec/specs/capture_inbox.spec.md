...
kind: feature
status: active
summary: The system allows users to create lightweight captures that enter an inbox state with source-level metadata preserved while remaining available to downstream retrieval workflows.
surface:
  - lib/hgs_brain
  - lib/hgs_brain_web
```
